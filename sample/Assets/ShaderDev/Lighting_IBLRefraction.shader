		...
		Pass
		{
			...
			vertexOutput vert(vertexInput v)
			{
				...
				o.normalWorld = float4(normalize(mul(normalize(v.normal.xyz), (float3x3)unity_WorldToObject)),v.normal.w);
					...
					float3 mainTexColor = tex2Dlod(_MainTex, o.texcoord);
			...
			half4 frag(vertexOutput i) : COLOR
			{
				float4 finalColor = float4(0, 0, 0, _Color.a);
					...
					float3 mainTexColor = tex2Dlod(_MainTex, i.texcoord);

					finalColor.rgb = mainTexColor * _Color * diffuseColor + specularColor;
